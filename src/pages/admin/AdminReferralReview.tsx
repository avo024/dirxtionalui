...
export default function AdminReferralReview() {
  ...
  // PA letter info used to arrive via the (now-retired) PA card's onChange
  // callback — fetch it directly instead, same as the workstation does.
  useEffect(() => {
    if (!id) return;
          ...
          {referral.status === 'processing' || isReExtracting ? (
            ...
            >
                  ...
                  {referral.is_bridge_program ? (
                    ...
                    <div style={{ padding: "4px 18px 16px" }}>
                      <p className="text-sm text-muted-foreground">
                        PA management moved to the{" "}
                        <button type="button" className="underline" onClick={() => navigate(`/admin/referrals/${id}`)}>
                          workstation
                        </button>
                        .
                      </p>
              ...
              <TabsContent value="all-fields">
                <ExtractionEditor referral={referral} onSaved={() => fetchReferralData()} />
              </TabsContent>
